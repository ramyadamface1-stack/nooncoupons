const STATE_KEY='maintenance/article-corpus-audit-v1.json';
...
function auditOne(key,html,md={}){
  const counters={},samples={},plain=strip(html),wc=words(plain).length,title=String(md.title||titleOf(html)||h1Of(html)||''),metaDesc=meta(html,'description'),canon=canonicalOf(html),h1=count(html,/<h1\b/gi),h2=count(html,/<h2\b/gi),h3=count(html,/<h3\b/gi),ps=paragraphStats(html);
  const imgs=[...String(html).matchAll(/<img\b[^>]*>/gi)].map(x=>x[0]),internal=count(html,/href=["']\//gi),lists=count(html,/<(?:ul|ol)\b/gi),tables=count(html,/<table\b/gi);
  ...
  ck('eeat',issue('eeat_editorial_review_missing',!/مراجعة تحريرية|المراجع|فريق تحرير/i.test(plain)));ck('eeat',issue('eeat_methodology_missing',!/<section\b[^>]*class=["'][^"']*(?:methodology|accountability)/i.test(html)));ck('eeat',issue('eeat_sources_section_missing',!/<section\b[^>]*class=["'][^"']*sources/i.test(html)));ck('eeat',issue('eeat_last_verified_missing',!/<time\b[^>]*datetime=/i.test(html)||!/آخر تحقق|آخر مراجعة/i.test(plain)));ck('eeat',issue('eeat_official_noon_source_missing',!/https:\/\/www\.noon\.com\//i.test(html)));ck('eeat',issue('eeat_unsupported_promo_claim',UNSUPPORTED.test(plain)));
  ck('geo',issue('geo_country_metadata_missing',!country));ck('geo',issue('geo_market_mention_missing',!!market&&!plain.includes(market)));ck('geo',issue('geo_wrong_market_mention',!!wrongCountry&&wrongCountry.test(plain)));ck('geo',issue('geo_currency_localization_missing',country==='SA'?!/(?:الريال السعودي|\bSAR\b)/i.test(plain):country==='AE'?!/(?:الدرهم الإماراتي|الدرهم الاماراتي|\bAED\b)/i.test(plain):false));ck('geo',issue('geo_market_internal_link_missing',country==='SA'?!/href=["']\/saudi(?:["']|\/)/i.test(html):country==='AE'?!/href=["']\/uae(?:["']|\/)/i.test(html):false));ck('geo',issue('geo_arabic_localization_low',arRatio<0.9&&arRatio>=0.5));
  ck('tech',issue('technical_article_tag_missing',!/<article\b/i.test(html)));ck('tech',issue('technical_section_missing',!/<section\b/i.test(html)));ck('tech',issue('technical_internal_links_lt5',internal<5));ck('tech',issue('technical_placeholder_links',/href=["'](?:#|javascript:)/i.test(html)));ck('tech',issue('technical_copy_cta_missing',!/data-copy-code=/i.test(html)));ck('tech',issue('technical_aria_live_missing',!/aria-live=["']polite/i.test(html)));
  ck('media',issue('media_images_lt4',imgs.length<4));ck('media',issue('media_missing_alt',imgs.some(x=>!/alt=["'][^"']{3,}["']/i.test(x))));ck('media',issue('media_missing_dimensions',imgs.some(x=>!/width=["']?\d+/i.test(x)||!/height=["']?\d+/i.test(x))));ck('media',issue('media_lazy_lt3',imgs.filter(x=>/loading=["']lazy["']/i.test(x)).length<3));ck('media',issue('media_figcaption_lt4',count(html,/<figcaption\b/gi)<4));
  ck('generative',issue('geoai_answer_first_missing',!/class=["'][^"']*direct-answer/i.test(html)));ck('generative',issue('geoai_source_transparency_missing',!/المصدر الرسمي|مصدر التحقق|نون الرسمي|https:\/\/www\.noon\.com\//i.test(html)));ck('generative',issue('geoai_methodology_missing',!/methodology|accountability|منهجية التحقق|طريقة التحقق/i.test(html)));ck('generative',issue('geoai_faq_missing',!/FAQPage|class=["'][^"']*faq/i.test(html)));ck('generative',issue('geoai_structured_comparison_missing',tables<1&&lists<2));ck('generative',issue('geoai_freshness_signal_missing',!/<time\b[^>]*datetime=/i.test(html)));ck('generative',issue('geoai_entity_clarity_missing',!/(?:نون|Noon)/i.test(plain)||!codes.length));
  ...
  issue('stored_schema_missing',jl.scripts===0);issue('stored_schema_parse_error',jl.parseErrors>0,jl.parseErrors);issue('stored_schema_article_missing',jl.scripts>0&&!jl.types.has('Article'));issue('stored_schema_faq_missing',jl.scripts>0&&!jl.types.has('FAQPage'));issue('stored_schema_breadcrumb_missing',jl.scripts>0&&!jl.types.has('BreadcrumbList'));
...
export async function runArticleCorpusAuditBatch(env,{limit=250,reset=false}={}){
  ...
  const page=await retry(()=>env.CONTENT_FINAL.list({prefix:'articles/',cursor:state.cursor||undefined,limit:Math.max(1,Math.min(Number(limit)||250,500)),include:['customMetadata']}));
  const objs=(page.objects||[]).filter(o=>String(o.key||'').endsWith('.html'));
  ...
  state.cursor=page.truncated?page.cursor:null;state.scanDone=!page.truncated;state.done=state.scanDone&&state.readFailures===0;state.lastBatchAt=new Date().toISOString();if(state.scanDone)state.completedAt=new Date().toISOString();
  await save(env,state);
  return {ok:true,batch:{objects:objs.length},...publicState(state)};
}
