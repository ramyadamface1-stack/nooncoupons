...
const MAPS=['/sitemap-pages.xml','/sitemap-articles.xml','/sitemap-categories.xml','/sitemap-coupons.xml','/sitemap-coupons-saudi.xml','/sitemap-coupons-uae.xml','/sitemap-stores.xml'];
...
if(path==='/sitemap-pages.xml')out=urls.filter(u=>!p(u).startsWith('/category/')&&!p(u).startsWith('/guide/')&&!p(u).startsWith('/articles/')&&!p(u).startsWith('/coupon/')).concat(TRUST.map(x=>ORIGIN+x),[ORIGIN+'/faq',ORIGIN+'/research',ORIGIN+'/glossary',ORIGIN+'/store/noon']);
if(path==='/sitemap-categories.xml')out=urls.filter(u=>p(u).startsWith('/category/'));
if(path==='/sitemap-articles.xml')out=urls.filter(u=>p(u).startsWith('/articles/')||p(u).startsWith('/guide/'));
