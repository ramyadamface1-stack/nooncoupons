...
export async function runProgrammaticBatch(env,cfg,status,{dailyTarget=2000,batchSize=3}={}){
  ...
  let cursor=Math.max(V2_CURSOR_START,Number(status.bulkCursorV2||0)),tries=0,rejectedQuality=0,rejectedDuplicate=0,rejectedGlobal=0,rejectedCoupon=0,rejectedLinks=0,rejectedSchema=0,rejectedIndexation=0;
  const records=[];
  while(records.length<batch&&countBefore+records.length<target&&tries<batch*18){
    ...
    const rec={
      ...
      editorialTrust:article.editorialTrust,
