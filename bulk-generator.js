...
export async function runProgrammaticBatch(env,cfg,status,{dailyTarget=2000,batchSize=2}={}){
  ...
  let cursor=Math.max(V2_CURSOR_START,Number(status.bulkCursorV2||0)),tries=0,rejectedQuality=0,rejectedDuplicate=0,rejectedGlobal=0,rejectedCoupon=0,rejectedLinks=0;
  const records=[];
  ...
  const patch={
    ...
    bulkLastError:records.length?'':(countBefore>=target?null:'quality_gate_no_article_passed'),bulkLastSlug:last?.slug||status.bulkLastSlug||null,
