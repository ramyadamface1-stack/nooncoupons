...
async function bulkBlogPage(req,env,ctx){
  ...
  const u=new URL(req.url),perPage=24,totalPages=Math.max(1,Math.ceil(all.length/perPage)),requested=Math.max(1,Number.parseInt(u.searchParams.get('page')||'1',10)||1),page=Math.min(requested,totalPages),offset=(page-1)*perPage,rows=all.slice(offset,offset+perPage);
