...
async function bulkArticlePage(u,env){
  ...
  const rawBody=await o.text(),decorated=decorateArticleBody(rawBody),body=decorated.body,canonical=origin+'/articles/'+encodeURI(slug),market=country==='SA'?'السعودية':'الإمارات',lang=country==='SA'?'ar-SA':'ar-AE',marketPath=country==='SA'?'/saudi':'/uae',published=md.at||now(),modified=o.uploaded?new Date(o.uploaded).toISOString():published;
  const imgPath=`/assets/coupon-svg/${encodeURIComponent(slug)}/1.svg?v=7&coupon=${encodeURIComponent(coupon)}&country=${country}`,featured=origin+imgPath;
  const images=[1,2,3,4,5].map(v=>origin+`/assets/coupon-svg/${encodeURIComponent(slug)}/${v}.svg?v=7&coupon=${encodeURIComponent(coupon)}&country=${country}`);
...
async function bulkBlogPage(req,env,ctx){
  ...
  const cards=all.slice(0,240).map(a=>{const country=a.country==='AE'?'AE':'SA',market=country==='SA'?'السعودية':'الإمارات',img=`/assets/coupon-svg/${encodeURIComponent(a.slug)}/1.svg?v=7&coupon=${encodeURIComponent(a.coupon||(country==='AE'?'OPS58':'OPS32'))}&country=${country}`,date=a.updatedAt||a.createdAt||'',dateText=date?new Intl.DateTimeFormat('ar-EG',{day:'numeric',month:'short',year:'numeric',timeZone:'Africa/Cairo'}).format(new Date(date)):'';return `<article class="card"><a class="thumb" href="/articles/${encodeURI(a.slug)}"><img src="${img}" alt="${esc(a.primaryKeyword||a.title)}" title="${esc(a.title||a.primaryKeyword||'')}" width="1200" height="760" loading="lazy" decoding="async"></a><div class="cardBody"><div class="cardMeta"><span class="tag">${market}</span><span>${esc(dateText)}</span></div><h2><a href="/articles/${encodeURI(a.slug)}">${esc(a.title)}</a></h2><p>${esc((a.metaDescription||'').slice(0,180))}</p><div class="cardFoot"><span>فريق تحرير كوبونات نون</span><a class="read" href="/articles/${encodeURI(a.slug)}">اقرأ الدليل ←</a></div></div></article>`}).join('');
  const origin=env.SITE_ORIGIN||new URL(req.url).origin;
  const h=`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>مدونة كوبونات نون | أحدث أدلة السعودية والإمارات</title><meta name="description" content="أحدث أدلة نون السعودية والإمارات: استخدام الكوبون، اختيار المنتجات، مقارنة السعر النهائي، وحل مشاكل الأهلية قبل الدفع."><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${origin}/blog"><style>:root{--ink:#101828;--muted:#667085;--line:#e7e9ee;--gold:#f5c400}*{box-sizing:border-box}body{margin:0;font-family:Tahoma,Arial;background:#f8fafc;color:var(--ink)}.w{width:min(1180px,94%);margin:auto}header{background:#0f172a;color:#fff;border-bottom:1px solid #243047}header .w{min-height:72px;display:flex;align-items:center;justify-content:space-between;gap:18px}header nav{display:flex;gap:18px;flex-wrap:wrap}header a{color:#fff;text-decoration:none;font-weight:800}.brand{font-size:21px}.hero{padding:54px 0 34px;background:radial-gradient(circle at 15% 10%,rgba(245,196,0,.17),transparent 30%),#fff;border-bottom:1px solid var(--line)}.hero small{font-weight:900;color:#7b6200}.hero h1{font-size:clamp(36px,5vw,58px);margin:10px 0}.hero p{max-width:760px;color:var(--muted);line-height:1.9;font-size:17px}.heroActions{display:flex;gap:10px;flex-wrap:wrap;margin-top:18px}.heroActions a{padding:10px 14px;border-radius:999px;border:1px solid var(--line);background:#fff;color:var(--ink);text-decoration:none;font-weight:900}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:18px;padding:34px 0 58px}.card{background:#fff;border:1px solid var(--line);border-radius:20px;overflow:hidden;box-shadow:0 14px 40px rgba(16,24,40,.06);transition:.18s}.card:hover{transform:translateY(-2px);box-shadow:0 20px 54px rgba(16,24,40,.10)}.thumb{display:block;background:#fffdf2;border-bottom:1px solid #edf0f4}.thumb img{display:block;width:100%;height:auto;aspect-ratio:1200/760;object-fit:contain}.cardBody{padding:17px}.cardMeta,.cardFoot{display:flex;align-items:center;justify-content:space-between;gap:10px;font-size:12px;color:var(--muted)}.tag{background:#fff8d5;color:#6f5700;border:1px solid #f0df83;padding:5px 9px;border-radius:999px;font-weight:900}.card h2{font-size:19px;line-height:1.62;margin:12px 0}.card h2 a,.read{text-decoration:none;color:var(--ink)}.card p{color:var(--muted);line-height:1.8;margin:0 0 16px}.read{font-weight:900;color:#5b21b6}.cardFoot{padding-top:12px;border-top:1px solid #f0f1f3}@media(max-width:900px){.grid{grid-template-columns:1fr 1fr}}@media(max-width:640px){header .w{display:block;padding:16px 0}header nav{margin-top:10px}.grid{grid-template-columns:1fr}.hero{padding-top:38px}}</style></head><body><header><div class="w"><a class="brand" href="/">كوبونات نون</a><nav><a href="/coupons">الكوبونات</a><a href="/saudi">السعودية</a><a href="/uae">الإمارات</a><a href="/editorial-policy">السياسة التحريرية</a></nav></div></header><section class="hero"><div class="w"><small>أدلة عملية قبل الدفع</small><h1>مدونة كوبونات نون</h1><p>أدلة شراء واستخدام للكوبونات في السعودية والإمارات، يراجعها فريق التحرير للتأكد من وضوح السوق والكود والمعلومات الأساسية قبل النشر.</p><div class="heroActions"><a href="/saudi/categories">تصفح أقسام السعودية</a><a href="/uae/categories">تصفح أقسام الإمارات</a><a href="/coupon-verification">كيف نتحقق من الأكواد؟</a></div></div></section><main class="w"><section class="grid">${cards}</section></main></body></html>`
