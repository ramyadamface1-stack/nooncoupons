...
export async function runLegacyUpgradeBatchV2(env,cfg,status,{batchSize=4}={}){
  ...
  const st=await readState(env),all=(st.articles||[]).filter(a=>a.status==='published'),remainingBefore=all.filter(a=>!String(a.provider||'').startsWith('workers-ai:')&&!String(a.provider||'').startsWith('programmatic-cloudflare:v2-legacy-upgrade')).length;
  if(!remainingBefore)return {ok:true,skipped:'legacy_upgrade_complete',records:[],patch:{legacyUpgradeComplete:true,legacyUpgradeRemaining:0,legacyUpgradeLastRun:now(),legacyUpgradeLastError:null,legacyUpgradeNeedsConsolidation:false}};
  ...
  const previousFailures=Array.isArray(status.legacyUpgradeFailureSamples)?status.legacyUpgradeFailureSamples:[],failureSamples=[...failures,...previousFailures].slice(0,12),upgradedTotal=Math.max(0,Number(status.legacyUpgradeTotal||0))+records.length,last=records.at(-1)||null;
