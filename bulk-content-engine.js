...
const UAE_GOLDEN_COMMERCIAL_MODIFIERS=['coupon today','coupon 2026','promo code 2026','discount code 2026','best deals','best offers','deals today','offers today','sale today','price UAE','best price UAE','Dubai deals','Abu Dhabi deals','Sharjah deals','buy online UAE'];
...
function uaeEnglishPriorityKeyword(candidate){
  ...
  const useDemand=demand.length>0&&seed%5!==0,head=useDemand?demand[seed%demand.length]:heads[seed%heads.length],goldenModifier=seed%10!==9,modifier=(goldenModifier?UAE_GOLDEN_COMMERCIAL_MODIFIERS:UAE_ENGLISH_QUERY_MODIFIERS)[Math.floor(seed/Math.max(1,heads.length))%(goldenModifier?UAE_GOLDEN_COMMERCIAL_MODIFIERS.length:UAE_ENGLISH_QUERY_MODIFIERS.length)],intentSuffix=UAE_INTENT_SUFFIX[candidate.intent]||'shopping guide';
