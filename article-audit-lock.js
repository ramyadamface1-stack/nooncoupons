...
export async function readArticleAuditLock(env){
  ...
  try{
    ...
    const x=JSON.parse(await o.text()),expires=Date.parse(x?.expiresAt||'');
    if(x?.active!==true)return {active:false,key:KEY,...x};
    if(Number.isFinite(expires)&&expires<=Date.now())return {active:false,expired:true,key:KEY,...x};
    return {active:true,key:KEY,...x};
  }catch(e){
