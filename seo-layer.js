...
function moneyPage(row){
  ...
  const graph={'@context':'https://schema.org','@graph':[{'@type':'WebPage','@id':ORIGIN+path,url:ORIGIN+path,name:title,description:desc,inLanguage:'ar'},{'@type':'FAQPage',mainEntity:[['هل الأكواد مضمونة لكل حساب؟','لا. أهلية الكود والخصم الفعلي يعتمدان على شروط نون والسلة والحساب وقت الطلب.'],['كيف أتحقق من الكود؟','ثبّت المنتجات والبائع، أدخل الكود في السلة، ثم قارن الإجمالي النهائي قبل وبعد التطبيق.'],['هل الصفحة خاصة بنفس الدولة؟',`نعم، هذه الصفحة مخصصة لنون ${country} ولا تنقل نتائج سوق آخر باعتبارها مؤكدة هنا.`]].map(([q,a])=>({'@type':'Question',name:q,acceptedAnswer:{'@type':'Answer',text:a}}))}]};
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title} | محدث تلقائيًا</title><meta name="description" content="${desc}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${ORIGIN+path}"><script type="application/ld+json">${JSON.stringify(graph).replace(/</g,'\\u003c')}</script><style>body{margin:0;font-family:Tahoma,Arial,sans-serif;background:#f8fafc;color:#111827}.w{width:min(1050px,92%);margin:auto}.hero{background:#111827;color:white;padding:52px 0}.hero h1{font-size:clamp(34px,6vw,58px);margin:10px 0}.hero p,.lead{line-height:1.9;color:#cbd5e1}.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px;padding:28px 0}.c{background:white;border:1px solid #e5e7eb;border-radius:18px;padding:20px}.c a{font-weight:900;color:#5b21b6}.answer{background:#fff7ed;border:1px solid #fed7aa;border-radius:18px;padding:18px;margin-top:24px;line-height:1.9}@media(max-width:650px){.grid{grid-template-columns:1fr}}</style></head><body><header class="hero"><div class="w"><a href="${marketPath}" style="color:#ddd6fe">نون ${country}</a><h1>${title}</h1><p>${desc}</p></div></header><main class="w"><section class="answer"><strong>الإجابة المختصرة:</strong> جرّب الأكواد أدناه على نفس السلة، ولا تعتمد على نسبة أو أهلية غير ظاهرة في متجر نون. الإجمالي النهائي داخل نون هو المرجع.</section><section class="grid">${codes}</section><section><h2>طريقة التحقق قبل الدفع</h2><p>اختر المنتج والبائع أولًا، سجّل إجمالي السلة، ثم جرّب الكود دون تغيير المنتجات. بعد التطبيق راجع الخصم والشحن والإجمالي النهائي. بهذه الطريقة تستطيع معرفة أثر الكود الحقيقي بدل الخلط بين تغير السعر وتغير القسيمة.</p><p><a href="/coupon-verification">منهجية التحقق من الأكواد</a> · <a href="/editorial-policy">السياسة التحريرية</a> · <a href="/coupons">كل الأكواد</a></p></section></main></body></html>`;
}
