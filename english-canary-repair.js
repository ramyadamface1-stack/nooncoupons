...
export async function repairEnglishCanaryLegacyMetadata(env){
  ...
  if(existing){
    try{const marker=await existing.json();if(marker?.complete)return {ok:true,skipped:'already_repaired',marker}}catch{}
  }
...
export async function englishCanaryRepairHealth(env){
  ...
  let marker=null;try{const o=await env.CONTENT_FINAL.get(MARKER_KEY);if(o)marker=await o.json()}catch{}
  ...
  return {ok:clean,version:'guide-guide-v1',targetCount:TARGET_SLUGS.length,totalStateRecords:rows.length,unexpectedEnglishRecords:unexpected,stateClean:stateChecks.every(x=>x.present&&x.clean),r2HtmlClean:articleChecks.every(x=>x.present&&x.htmlClean),r2MetadataClean:articleChecks.every(x=>x.present&&x.metadataClean),state:stateChecks,articles:articleChecks,marker};
}
...
export const ENGLISH_CANARY_REPAIR_INFO={version:'guide-guide-v1',targets:[...TARGET_SLUGS],stateKey:STATE_KEY,markerKey:MARKER_KEY};
