...
function mobileCouponBar(html,noonUrl,fallbackCode=''){
  ...
  if(!/^OPS\d{2}$/.test(code)&&/^OPS\d{2}$/.test(String(fallbackCode||'').toUpperCase()))code=String(fallbackCode).toUpperCase();
...
async function publicView(req,res,env){
  ...
  if(path.startsWith('/articles/')){
    ...
    let fallbackCode='';
    if(!/\bOPS\d{2}\b/i.test(html)){
      try{
        const health=env?.CONTENT_FINAL?await env.CONTENT_FINAL.get('bulk/latest.json'):null;
        const latest=health?await health.json():null;
        const rec=(latest?.articles||[]).find(a=>a?.slug===decodeURIComponent(path.split('/').pop()||''));
        fallbackCode=String(rec?.code||rec?.couponCode||'').toUpperCase();
      }catch{}
    }
    html=mobileCouponBar(html,noonUrl,fallbackCode);
