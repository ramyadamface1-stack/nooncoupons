...
const summary={samples:samples.length,attempts,minScore:Math.min(...samples.map(x=>x.audit.score)),minWords:Math.min(...samples.map(x=>x.audit.wordCount)),maxWords:Math.max(...samples.map(x=>x.audit.wordCount)),minJaccardDistance:Math.min(...samples.map(x=>x.audit.minJaccardDistance)),minEnglishRatio:Math.min(...samples.map(x=>x.audit.englishRatio)),distanceDistribution:samples.map(x=>({keyword:x.article.primaryKeyword,d:x.audit.minJaccardDistance})),failures:bad.map(x=>({keyword:x.article.primaryKeyword,score:x.audit.score,words:x.audit.wordCount,p0:x.audit.p0,failed:x.audit.failed,groups:x.audit.groups}))};
