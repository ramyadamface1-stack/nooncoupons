...
const DEFAULT_DAILY_TARGET=5760;
const MAX_DAILY_TARGET=5760;
...
function batchSizeFromEnv(env){const requested=Number(env.ENGLISH_BATCH_SIZE||4)||4;return Math.max(1,Math.min(4,requested))}
...
async function findCandidate(env,state,slot){
  ...
  const diag={scanned:0,wrongCountry:0,nonPriorityProfile:0,nonHighDemandProfile:0,missingCategory:0,noNativeCandidate:0,invalidCoupon:0,profileCap:0,intentCap:0,noArticle:0,stateDuplicate:0,r2Duplicate:0,auditNotReady:0,jaccardTooLow:0,auditReasons:{}};
  ...
  for(let tries=0;tries<MAX_SCAN;tries++,cursor++){
    ...
    if(slot%4!==3&&!UAE_HIGH_DEMAND_PROFILES.has(topic.profileKey)){diag.nonHighDemandProfile++;continue;}
    ...
    if(await env.CONTENT_FINAL.head('articles/'+article.slug+'.html')){diag.r2Duplicate++;continue;}
...
export const ENGLISH_CANARY_INFO={version:11,evidenceSafeSummary:true,visibleEditorialByline:true,richArticleSchema:true,indexNowOnPublish:true,indexNowUrlPathAware:true,maxArticles:2,controlledMaxArticles:MAX_CONTROLLED_TOTAL,dailyMaxArticles:MAX_DAILY_TARGET,batchMaxPerTick:4,defaultBatchPerTick:4,highDemandUaeKeywordTargeting:true,controlledTargetMarket:TARGET_MARKET,marketPolicy:'uae-only-new-v4-batch4',uaePriorityProfiles:[...UAE_PRIORITY_PROFILES],uaeHighDemandProfiles:[...UAE_HIGH_DEMAND_PROFILES],highDemandShareTarget:75,diversityWindow:DIVERSITY_WINDOW,stateKey:STATE_KEY,builder:6,route:'/en/articles/:slug',sitemap:'/sitemap-en-articles.xml',qualityThreshold:95,jaccardThreshold:0.18};
