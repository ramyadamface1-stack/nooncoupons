...
const xmlResponse=s=>new Response(s,{headers:{'content-type':'application/xml; charset=utf-8','cache-control':'public,max-age=30,s-maxage=60,stale-while-revalidate=120'}});
...
function batchSizeFromEnv(env){const requested=Number(env.ENGLISH_BATCH_SIZE||24)||24;return Math.max(1,Math.min(24,requested))}
...
async function findCandidate(env,state,slot,{deadlineMs=0,maxScan=MAX_SCAN}={}){
  ...
  const diag={scanned:0,wrongCountry:0,nonPriorityProfile:0,nonHighDemandProfile:0,missingCategory:0,noNativeCandidate:0,invalidCoupon:0,keywordMixSkip:0,profileCap:0,intentCap:0,noArticle:0,stateDuplicate:0,scanDuplicate:0,r2Duplicate:0,r2HeadRateLimited:0,r2HeadRetryAttempts:0,auditNotReady:0,jaccardTooLow:0,auditReasons:{}};
  const seenCandidateSlugs=new Set();
  let cursor=Math.max(START_CURSOR,Number(state.cursor||START_CURSOR));
  const scanLimit=Math.max(1,Math.min(MAX_SCAN,Number(maxScan||MAX_SCAN)||MAX_SCAN));
  for(let tries=0;tries<scanLimit;tries++,cursor++){
    if(deadlineMs&&Date.now()>=deadlineMs){diag.budgetExhausted=true;return {notFound:true,budgetExhausted:true,diagnostics:diag,nextCursor:cursor}}
    diag.scanned++;
    ...
    if(!rawCandidate){diag.noNativeCandidate++;continue;}
    ...
    if(slot<PROFILE_DIVERSITY_SLOTS&&usedProfiles.has(candidate.profileKey)){diag.profileCap++;continue;}
    if((profileCounts.get(candidate.profileKey)||0)>=PROFILE_MAX_PER_BATCH){diag.profileCap++;continue;}
    if((intentCounts.get(candidate.intent)||0)>=INTENT_MAX_PER_BATCH){diag.intentCap++;continue;}
    const article=buildEnglishUsefulArticle(candidate,cursor);
    if(!article){diag.noArticle++;continue;}
    if(usedSlugs.has(article.slug)||usedKeywords.has(String(article.primaryKeyword||'').toLowerCase())){diag.stateDuplicate++;continue;}
    if(seenCandidateSlugs.has(article.slug)){diag.scanDuplicate++;continue;}
    seenCandidateSlugs.add(article.slug);
    const audit=auditEnglishSeoArticle(article,candidate,{recent,threshold});
    if(!audit.productionReady){diag.auditNotReady++;for(const reason of audit?.failed||audit?.reasons||audit?.issues||[]){const key=String(reason?.code||reason?.reason||reason||'unknown').slice(0,120);diag.auditReasons[key]=(diag.auditReasons[key]||0)+1}continue;}if(audit.minJaccardDistance<0.18){diag.jaccardTooLow++;continue;}
...
export async function runEnglishCanary(env,{ignoreInterval=false,scanBudgetMs=5000}={}){
  ...
  const write=await r2PutArticleCreateOnly(env,'articles/'+article.slug+'.html',String(article.html||''),{
  ...
  if(write.duplicate){
    const existingMeta=write?.existing?.customMetadata||{},existingLang=String(existingMeta.lang||'').toLowerCase(),existingProvider=String(existingMeta.p||existingMeta.provider||'').toLowerCase();
    const recoverableExisting=write.recovered||existingLang==='en'||existingProvider.includes('english-v6')||(String(article.slug||'').startsWith('en-')&&candidate.country==='AE');
    if(recoverableExisting){
...
export async function englishCanaryHealth(env){
  ...
  for(let i=0;i<sampleBase.length;i+=8){
    const chunk=await Promise.all(sampleBase.slice(i,i+8).map(async r=>{let r2Present=false;try{r2Present=Boolean(await env.CONTENT_FINAL?.head('articles/'+r.slug+'.html'))}catch{}return {...r,r2Present}}));
    verified.push(...chunk);
  }
  const verifiedBySlug=new Map(verified.map(r=>[r.slug,r])),records=canaryBase.map(r=>verifiedBySlug.get(r.slug)||{...r,r2Present:false});
  ...
  const today=publishedToday(allRecords),dailyTarget=dailyTargetFromEnv(env),minIntervalMinutes=minIntervalMinutesFromEnv(env),batchSize=batchSizeFromEnv(env),missing=verified.filter(r=>!r.r2Present).length;
...
function englishRelatedBlock(state,rec,countryPath,categoryPath){
  const marketLabel=rec?.country==='SA'?'Saudi Arabia':'UAE';
  const rows=(state?.records||[]).slice(-600).filter(r=>r?.slug&&r.slug!==rec.slug&&r.country===rec.country&&r.status==='published'&&r.indexable!==false&&r.languageSource==='native-intent-v6-canary'&&Number(r.quality)>=95&&Number(r.qualityFloor)>=88&&r.superseded!==true&&r.orphan!==true&&r.duplicateIntent!==true);
  const scored=rows.map((r,i)=>({r,i,s:(r.categoryKey&&rec.categoryKey&&r.categoryKey===rec.categoryKey?8:0)+(r.intent&&rec.intent&&r.intent===rec.intent?5:0)+(r.keywordCluster&&rec.keywordCluster&&r.keywordCluster===rec.keywordCluster?4:0)+(r.profileKey&&rec.profileKey&&r.profileKey===rec.profileKey?3:0)})).sort((a,b)=>b.s-a.s||String(b.r.updatedAt||b.r.createdAt||'').localeCompare(String(a.r.updatedAt||a.r.createdAt||''))||b.i-a.i).slice(0,6).map(x=>x.r);
  if(!scored.length)return {html:'',count:0};
  const cards=scored.map(r=>`<article class="related-en-card"><small>${esc(r.keywordTier||r.intent||(marketLabel+' shopping guide'))}</small><h3><a href="/en/articles/${enc(r.slug)}">${esc(r.title||r.primaryKeyword||r.slug)}</a></h3><p>${esc(String(r.metaDescription||'').slice(0,150))}</p><a class="related-en-read" href="/en/articles/${enc(r.slug)}">Read guide →</a></article>`).join('');
  const html=`<section id="english-related-network" aria-label="Related English guides"><style id="english-related-network-css">#english-related-network{margin-top:34px;padding-top:28px;border-top:1px solid #e5e7eb}.related-en-head{display:flex;align-items:end;justify-content:space-between;gap:12px}.related-en-head h2{margin:0}.related-en-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-top:16px}.related-en-card{border:1px solid #e5e7eb;border-radius:14px;padding:15px;background:#fff}.related-en-card small{color:#6d28d9;font-weight:800}.related-en-card h3{margin:7px 0;font-size:17px;line-height:1.45}.related-en-card a{text-decoration:none}.related-en-card p{font-size:14px;color:#64748b}.related-en-read{font-weight:900}.related-en-links{display:flex;gap:10px;flex-wrap:wrap;margin-top:16px}.related-en-links a{font-weight:800}@media(max-width:650px){.related-en-grid{grid-template-columns:1fr}.related-en-head{display:block}}</style><div class="related-en-head"><div><small>Keep comparing</small><h2>Related Noon ${marketLabel} shopping guides</h2></div><a href="${countryPath}">${marketLabel} English hub</a></div><div class="related-en-grid">${cards}</div><nav class="related-en-links"><a href="${categoryPath}">Category hub</a><a href="/coupons">Current coupon codes</a><a href="/coupon-verification">Coupon verification</a></nav></section>`;
  return {html,count:scored.length};
}
...
export async function serveEnglishCanaryArticle(req,env){
  ...
  const graph={'@context':'https://schema.org','@graph':[
    ...
    {'@type':'WebPage','@id':canonical+'#webpage',url:canonical,name:rec.title,description:rec.metaDescription,inLanguage:marketLang,isPartOf:{'@id':origin+'/#website'},breadcrumb:{'@id':canonical+'#breadcrumb'}},
    {'@type':'Article','@id':canonical+'#article',headline:rec.title,description:rec.metaDescription,inLanguage:marketLang,datePublished:rec.createdAt,dateModified:rec.updatedAt||rec.createdAt,mainEntityOfPage:{'@id':canonical+'#webpage'},author:{'@id':origin+'/authors/editorial-team#team'},publisher:{'@id':origin+'/#organization'},about:[{'@type':'Thing',name:'Noon'},{'@type':'Place',name:countryLabel},{'@type':'Thing',name:rec.primaryKeyword},{'@type':'Thing',name:'Noon coupon code '+safeCoupon,identifier:safeCoupon}]}
  ]};
  const safeMeta=replaceUnapprovedCouponTokens(rec.metaDescription,safeCoupon);const head=`<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(rec.title)}</title><meta name="description" content="${esc(safeMeta)}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${esc(canonical)}"><link rel="alternate" hreflang="${marketLang}" href="${esc(canonical)}"><link rel="alternate" hreflang="${arabicLang}" href="${esc(origin+arabicPath)}"><link rel="alternate" hreflang="x-default" href="${esc(canonical)}"><meta property="og:type" content="article"><meta property="og:locale" content="${rec.country==='SA'?'en_SA':'en_AE'}"><meta property="og:title" content="${esc(rec.title)}"><meta property="og:description" content="${esc(rec.metaDescription)}"><meta property="og:url" content="${esc(canonical)}"><meta property="og:site_name" content="NoonCoupons"><script type="application/ld+json" data-schema="article">${safeJson(graph)}</script>${pageCss()}`;
  ...
  const publishedLabel=new Intl.DateTimeFormat('en-GB',{day:'2-digit',month:'short',year:'numeric',timeZone:'Africa/Cairo'}).format(new Date(rec.createdAt));
  const byline=`<aside class="editorial-byline-en" data-english-author="editorial-team" aria-label="Editorial information"><strong>Reviewed by <a href="/authors/editorial-team">Noon Deals Now Editorial Team</a></strong><span> · Published ${esc(publishedLabel)} · Updated ${esc(updatedLabel)}</span><small>Independent guidance published under the <a href="/editorial-policy">editorial policy</a> and <a href="/coupon-verification">coupon verification methodology</a>. Checkout remains the final reference for eligibility and savings.</small></aside>`;
  const summary=`<aside class="coupon-summary-en" aria-label="Coupon summary"><strong>Coupon summary</strong><dl><div><dt>Code</dt><dd>${esc(safeCoupon)}</dd></div><div><dt>Market</dt><dd>${esc(countryLabel)}</dd></div><div><dt>Discount</dt><dd>Verify at checkout</dd></div><div><dt>Updated</dt><dd>${esc(updatedLabel)}</dd></div></dl><small>This code is on the site owner-approved allowlist for testing. We do not claim a fixed percentage, expiry date, or guaranteed eligibility without supporting evidence.</small></aside>`;
  const related=englishRelatedBlock(state,rec,countryPath,categoryPath);
  const body=`<header class="top"><div class="topin"><div class="brand"><a href="/en/${rec.country==='SA'?'saudi':'uae'}">NoonCoupons</a></div><nav><a href="${countryPath}">${countryLabel}</a><a href="${categoryPath}">Category</a><a href="/${rec.country==='SA'?'saudi':'uae'}">العربية</a></nav></div></header><main class="wrap"><div class="canary-note">Native English guide · ${phaseLabel} · quality-gated · coupon-first</div>${byline}${summary}${raw}${related.html}</main>${copyJs()}`;
  return new Response(`<!doctype html><html lang="${marketLang}" dir="ltr"><head>${head}</head><body>${body}</body></html>`,{headers:{'content-type':'text/html; charset=utf-8','cache-control':'public,max-age=60,s-maxage=300','content-language':'en','x-english-canary':'v1','x-english-phase':rec.canary?'canary':'controlled','x-english-related-count':String(related.count),'x-robots-tag':'index, follow','x-content-type-options':'nosniff'}});
}
