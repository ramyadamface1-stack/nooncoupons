...
async function findCandidate(env,state,slot,{deadlineMs=0,maxScan=MAX_SCAN}={}){
  ...
  for(let tries=0;tries<scanLimit;tries++,cursor++){
    ...
    const tier=String(candidate.keywordTier||''),isGolden=tier==='uae-golden-commercial-head-v4',isDemand=tier==='uae-high-demand-product-v4';
    if(wantsGolden){
      if(!isGolden||!UAE_GOLDEN_COMMERCIAL_INTENTS.has(candidate.intent)){diag.keywordMixSkip++;continue;}
    }else if(!isDemand){diag.keywordMixSkip++;continue;}
