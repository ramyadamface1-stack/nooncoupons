...
async function findCandidate(env,state,slot){
  ...
  for(let tries=0;tries<MAX_SCAN;tries++,cursor++){
    ...
    const head=await r2HeadSafe(env,'articles/'+article.slug+'.html');
    diag.r2HeadRetryAttempts+=Math.max(0,Number(head.attempts||1)-1);
    if(head.rateLimited){diag.r2HeadRateLimited++;continue;}
    if(head.exists){diag.r2Duplicate++;continue;}
...
export async function runEnglishCanaryBatch(env){
  const maxPerTick=batchSizeFromEnv(env),results=[];
  ...
  return {ok:results.some(r=>r?.ok!==false)||published>0,maxPerTick,published,results};
}
...
export const ENGLISH_CANARY_INFO={version:15,evidenceSafeSummary:true,visibleEditorialByline:true,richArticleSchema:true,indexNowOnPublish:true,indexNowUrlPathAware:true,maxArticles:2,controlledMaxArticles:MAX_CONTROLLED_TOTAL,dailyMaxArticles:MAX_DAILY_TARGET,batchMaxPerTick:16,defaultBatchPerTick:16,highDemandUaeKeywordTargeting:true,hardTargetMarketNormalization:true,controlledTargetMarket:TARGET_MARKET,marketPolicy:'uae-only-new-v6-batch16',uaePriorityProfiles:[...UAE_PRIORITY_PROFILES],uaeHighDemandProfiles:[...UAE_HIGH_DEMAND_PROFILES],highDemandShareTarget:80,goldenHeadShareTarget:90,r2HeadRetry:true,r2HeadRateLimitFailClosed:true,perScanSlugDedup:true,schedulerObservability:true,auditLockHealth:true,diversityWindow:DIVERSITY_WINDOW,stateKey:STATE_KEY,builder:6,route:'/en/articles/:slug',sitemap:'/sitemap-en-articles.xml',qualityThreshold:95,jaccardThreshold:0.18};
