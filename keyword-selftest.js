...
for(let i=0;i<160;i++){
  ...
  const problems=[];
  if(t.keywordStrategy!=='search-intent-v5-diversity')problems.push('wrong_strategy');
  ...
  seen.add(t.slug);
  rows.push({cursor:600000+i,keyword:t.kw,intent:t.intent,profile:t.profileKey,words,market:t.market,country:t.country,categoryKey:t.categoryKey,brandKey:t.brandKey,modelKey:t.modelKey,landingPath:t.landingPath,problems});
...
if(KEYWORD_STRATEGY_INFO.version!=='search-intent-v5-diversity')throw new Error('wrong_keyword_strategy_version');
