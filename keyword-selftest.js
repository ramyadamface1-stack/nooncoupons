...
for(let i=0;i<160;i++){
  ...
  if(t.catalogLevel!=='product-intent'&&hierarchyTarget&&!String(t.kw||'').includes(hierarchyTarget))problems.push('hierarchy_target_missing');
  if(t.catalogBrandKey&&t.brandKey!==t.catalogBrandKey)problems.push('catalog_brand_route_mismatch');
  if(t.catalogModelKey&&t.modelKey!==t.catalogModelKey)problems.push('catalog_model_route_mismatch');
  ...
  seen.add(t.slug);
  rows.push({cursor:600000+i,keyword:t.kw,intent:t.intent,profile:t.profileKey,words,market:t.market,country:t.country,categoryKey:t.categoryKey,catalogLevel:t.catalogLevel,catalogTarget:t.catalogTarget,catalogBrandKey:t.catalogBrandKey,catalogModelKey:t.catalogModelKey,seasonalTerm:t.seasonalTerm,brandKey:t.brandKey,modelKey:t.modelKey,landingPath:t.landingPath,problems});
