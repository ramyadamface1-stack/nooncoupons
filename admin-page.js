...
async function refreshMetrics(freshCount=false){
  ...
  $('cfgModel').value=h.workersAI?.model||d.config?.model||'';$('cfgTarget').value=d.config?.targetWords||1500;$('cfgMin').value=d.config?.minWords||1000;$('cfgQuality').value=d.config?.qualityThreshold||95;
}
...
async function refreshArticles(){
  ...
  $('articleRows').innerHTML=rows.articles.slice(0,500).map(x=>{
    ...
    return '<tr><td>'+escHtml(x.title)+'</td><td>'+escHtml(x.country)+'</td><td>'+escHtml(x.coupon)+'</td><td>'+escHtml(words)+'</td><td>'+escHtml(x.quality)+'</td><td>'+escHtml(fmtArticleDate(created))+'</td><td>'+escHtml(fmtArticleDate(updated))+'</td><td>'+escHtml(x.provider||'-')+'</td><td>'+escHtml(x.status)+'</td><td><button data-edit="'+encodeURIComponent(x.slug)+'">تعديل</button></td></tr>';
  }).join('');
